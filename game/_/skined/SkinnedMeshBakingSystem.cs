...
[assembly: RegisterGenericComponentType(typeof(DynamicBuffer<An_BoneEntity>))]
    ...
    internal class SkinnedMeshBaker : Baker<SkinnedMeshAuthoring>
    {
        public override void Bake(SkinnedMeshAuthoring authoring)
        {
            ...
            if (skinnedMeshRenderer == null)
                return;
            AddComponent<An_SkinnedMeshTag>();
            ...
            if (hasSkinning)
            {
                ...
                AddComponent(new An_RootEntity { Value = rootEntity });

                // Setup reference to the other bones
                var boneEntityArray = AddBuffer<An_BoneEntity>();
                ...
                for (int boneIndex = 0; boneIndex < skinnedMeshRenderer.bones.Length; ++boneIndex)
                {
                    ...
                    boneEntityArray[boneIndex] = new An_BoneEntity { Value = boneEntity };
                }

                // Store the bindpose for each bone
                var bindPoseArray = AddBuffer<An_BindPose>();
                ...
                for (int boneIndex = 0; boneIndex != skinnedMeshRenderer.bones.Length; ++boneIndex)
                {
                    ...
                    bindPoseArray[boneIndex] = new An_BindPose { Value = bindPose };
                }
            }
        }
    }
    ...
    public partial struct ComputeSkinMatricesBakingSystem : ISystem
    {
        ...
        public void OnCreate(ref SystemState state)
        {
            m_entityQuery = new EntityQueryBuilder(Allocator.Temp)
                .WithAll<An_SkinnedMeshTag, An_RootEntity, An_BoneEntity>()
        ...
        private partial struct ComputeSkinMatricesBakingDataJob : IJobEntity
        {
            ...
            [BurstCompile]
            public void Execute([ChunkIndexInQuery] int chunkIndex, in An_RootEntity anRootEntity,
                in DynamicBuffer<An_BoneEntity> bones)
            {
                ecb.AddComponent<LocalToWorld>(chunkIndex, anRootEntity.Value); // this is possibly redundant
                ecb.AddComponent<An_RootEntity>(chunkIndex, anRootEntity.Value);
                ...
                for (int boneIndex = 0; boneIndex < bones.Length; ++boneIndex)
                {
                    ...
                    ecb.AddComponent(chunkIndex, boneEntity, new An_BoneTag());
                }
            }
        }
    }
