...
namespace AnimationSystem
{
    ...
    public partial struct CalculateSkinMatrixSystem : ISystem
    {
        ...
        private partial struct CalculateSkinMatricesJob : IJobEntity
        {
            ...
            [BurstCompile]
            public void Execute(ref DynamicBuffer<SkinMatrix> skinMatrices, in DynamicBuffer<An_BindPose> bindPoses,
                    in DynamicBuffer<An_BoneEntity> bones, in An_RootEntity anRootEntityComponent)
            {
                // Loop over each bone
                for (int i = 0; i < skinMatrices.Length; ++i)
                {
                    ...
                    var rootEntity = anRootEntityComponent.Value;
                    ...
                    //bindPose = math.mul(m_lookup_LocalToWorld[boneEntity].Value, bindPose);
