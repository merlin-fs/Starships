...
{
    public abstract partial class RenderBonesBase : SystemBase
    {
        EntityQuery m_Query;
        ...
        static void ValidateMathTypeSizesAreEqual()
        {
            if (UnsafeUtility.SizeOf<float4>() != UnsafeUtility.SizeOf<Vector4>())
                throw new InvalidOperationException("Size mismatch between float4 and Vector4.");
            if (UnsafeUtility.SizeOf<float4x4>() != UnsafeUtility.SizeOf<Matrix4x4>())
                throw new InvalidOperationException("Size mismatch between float4x4 and Matrix4x4.");
        }
    }
}
